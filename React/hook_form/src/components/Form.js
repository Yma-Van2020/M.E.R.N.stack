...
const HookForm = (props) => {
  ...
  const handlecp = (e) => {
    ...
    if (e.target.value.length < 8 || e.target.value !== inp.password) {
      cpsetError(
        "field must be at least 8 characters and matches the previous one"
      );
    } else {
      cpsetError("");
    }
