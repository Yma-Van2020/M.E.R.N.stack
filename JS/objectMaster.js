...
const justNamesId99 = pokémon.filter(p => p.id > 99).map( p => p.name)
console.log(justNamesId99 )

const poisonP = pokémon.filter(p => p.types.length == 1 && p.types[0] === "poison").map(p => p.name)
console.log(poisonP)

const secondFly = pokémon.filter(p => p.types[1] === "flying").map(p => p.types[0])
